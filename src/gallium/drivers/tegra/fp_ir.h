...
void
tegra_fp_pack_alu(uint32_t *dst, struct fp_alu_instr *instr);

uint32_t
tegra_fp_pack_dw(struct fp_dw_instr *instr);

void
tegra_fp_pack_mfu(uint32_t *dst, struct fp_mfu_instr *instr);

uint32_t
tegra_fp_pack_sched(struct fp_sched *sched);
